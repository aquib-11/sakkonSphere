...
const CreateArticleModel = ({ setShowModal, createArticlePage, articleId, type, fetchArticles }) => {
  ...
  const handleSubmit = async () => {
    try {
      const { data } = await customFetch.post("/articles/create-article", { coverPage: content, title });
      setShowModal(false);
  ...
  return (
    <div className="flex gap-4 h-[95vh]">
      ...
      <div className="w-1/4 bg-gray-50 p-4 rounded-lg border border-gray-200 shadow-sm overflow-y-auto">
      ...
      <div className="w-2/4 flex flex-col gap-4">
        ...
        <div className="flex justify-end gap-4">
          ...
          <button
            onClick={type === "createPage" ? () => createArticlePage(articleId, content, Number(pageNumber)) : handleSubmit}
            className="px-4 py-2 rounded-lg bg-[var(--primary)] hover:bg-[var(--ternery)] text-white"
          >
            Submit
          </button>
        </div>
